...
UnicodeMap *UnicodeMap::parse(GString *encodingNameA)
{
	...
	{
		if ((tok1 = strtok_s(buf, " \t\r\n", &next_token1)) &&
			(tok2 = strtok_s(NULL, " \t\r\n", &next_token2)))
		{
			if (!(tok3 = strtok_s(NULL, " \t\r\n", &next_token3)))
			{
				tok3 = tok2;
				tok2 = tok1;
			}
			...
			if (nBytes <= 4)
			{
				...
				sscanf_s(tok1, "%x", &range->start);
				sscanf_s(tok2, "%x", &range->end);
				sscanf_s(tok3, "%x", &range->code);
				range->nBytes = nBytes;
				++map->len;
			}
			else if (tok2 == tok1)
			{
				...
				sscanf_s(tok1, "%x", &eMap->u);
				for (i = 0; i < nBytes; ++i)
				{
					sscanf_s(tok3 + i * 2, "%2x", &x);
					eMap->code[i] = (char)x;
				}
				eMap->nBytes = nBytes;
				++map->eMapsLen;
