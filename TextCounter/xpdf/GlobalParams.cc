...
int SysFontInfo::match(GString *nameA)
{
	...
	if (_strnicmp(name->getCString(), nameA->getCString(), 2))
	{
		return 0;
	}
		...
		&regKey) == ERROR_SUCCESS)
	{
		...
		while (1)
		{
				...
				dataLen > 0 && dataLen < sizeof(data))
			{
				...
				data[dataLen] = '\0';
				n = (int)strlen(data);
				if (!_stricmp(data + n - 4, ".ttf") ||
					!_stricmp(data + n - 4, ".ttc") ||
					!_stricmp(data + n - 4, ".otf"))
				{
	...
	char *path)
{
	...
	SysFontType type;
	if (!_stricmp(path + strlen(path) - 4, ".ttc"))
	{
		type = sysFontTTC;
	}
	else if (!_stricmp(path + strlen(path) - 4, ".otf"))
	{
		type = sysFontOTF;
	}
	else
...
GlobalParams::GlobalParams(const char *cfgFileName)
{
	...
	if (cfgFileName && cfgFileName[0])
	{
		...
		errno_t err = fopen_s(&f, fileName->getCString(), "r");
		if (err != 0)
		{
			delete fileName;
		}
...
static void getWinFontDir(char *winFontDir)
{
	...
	if (!winFontDir[0])
	{
		...
		strcpy_s(winFontDir + i, sizeof(winFontDir + i), "Fonts");
	}
}
#endif
...
void GlobalParams::setupBaseFonts(const char *dir)
{
	...
	for (i = 0; displayFontTab[i].name; ++i)
	{
		...
		if (dir)
		{
			...
			errno_t err = fopen_s(&f, fileName->getCString(), "rb");
