	...
	GString *userPassword, PDFCore *coreA)
{
	...
	errno_t err = fopen_s(&file, fileName1->getCString(), fopenReadMode);

	if (err != 0)
	{
		fileName2 = fileName->copy();
		fileName2->lowerCase();
		errno_t err2 = fopen_s(&file, fileName2->getCString(), fopenReadMode);
		if (err2 != 0)
		{
			fileName2->upperCase();
			errno_t err3 = fopen_s(&file, fileName2->getCString(), fopenReadMode);
			if (err3 != 0)
			{
	...
	GString *userPassword, PDFCore *coreA)
{
	...
	//} else {
	errno_t err = fopen_s(&file, fileName->getCString(), fopenReadMode);
...
GBool PDFDoc::saveAs(GString *name)
{
	...
	char buf[4096];
	int n;
	errno_t err = fopen_s(&f, name->getCString(), "wb");
	if (err != 0)
	{
...
GBool PDFDoc::saveEmbeddedFile(int idx, const wchar_t *path, int pathLen)
{
	...
	errno_t err = fopen_s(&f, path2c, "wb");
