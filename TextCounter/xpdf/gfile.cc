...
GString *getHomeDir()
{
	...
	getenv_s(&requiredSize, libvar, requiredSize, "HOME");

	if (requiredSize != 0)
		ret = new GString(libvar);
	else
		ret = new GString(".");
	return ret;
	...
	//auto x = new GString(err == 0 ? pValue : ".");
	...
	const char *mode, const char *ext)
{
	...
	for (i = 0; i < 1000; ++i)
	{
		...
		errno_t err = fopen_s(&f2, s2->getCString(), "r");

		if (err != 0)
		{
			errno_t err2 = fopen_s(&f2, s2->getCString(), mode);
			if (err2 != 0)
			{
				delete s2;
				delete s;
...
FILE *openFile(const char *path, const char *mode)
{
	...
	FILE *stream;
	errno_t err = _wfopen_s(&stream, wPath, wMode);
	return stream;
#elif defined(VMS)
...
void readWindowsShortcut(wchar_t *wPath, size_t wPathSize)
{
	...
	wcscpy_s(wPath, sizeof wPath, target);
}
#endif
