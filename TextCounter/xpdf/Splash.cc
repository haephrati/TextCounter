	   ...
	   //      (or backdrop color, for knockout groups)
			...
			switch (bitmap->mode)
			{
				...
				case splashModeCMYK8:
					...
					{   // non-isolated group
						if (color0Ptr)
						{ //   non-isolated, knockout group
							aPrev = *alpha0Ptr;
						}
						else
						{         //   non-isolated, non-knockout group
					...
					{           // isolated group
						if (color0Ptr)
						{ //   isolated, knockout group
							aPrev = 0;
						}
						else
						{         //   isolated, non-knockout group
							aPrev = aDest;
						}
					}
...
void Splash::useDestRow(int y)
{
	...
	for (yy = y0; yy <= y1; ++yy)
	{
		...
		{ // (groupDestInitMode == splashGroupDestInitCopy)
...
class BasicImageScaler : public ImageScaler
{
	...
	virtual Guchar *colorData()
	{
		return colorLine;
	}
	virtual Guchar *alphaData()
	{
		return alphaLine;
	}
